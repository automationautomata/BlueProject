...
def query_for_table(table: str, cols: list[str], interval: tuple[int, int], order_column: str, desc: bool):
    ...
    return f'''SELECT {str(cols)[1:-1]}, ROW_NUMBER() OVER (ORDER BY {order_column}) AS RowNum 
                            FROM {table} WHERE RowNum BETWEEN {interval[0]} AND {interval[1]} 
                                        ORDER BY {order_column} {'DESC' if desc else ''};'''
